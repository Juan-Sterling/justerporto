...
export default function TechBadge({ name, size = 'default' }) {
  ...
  return (
    ...
    >
      {/* Icon Badge */}
      <div
        className={`${containerSize} rounded-md bg-[#090909] border transition-all duration-200 flex items-center justify-center p-1.5 cursor-pointer ${
          isHovered
            ? 'border-[#E11D2E] bg-[#1C1C1C] -translate-y-0.5 shadow-md shadow-black/70 ring-1 ring-[#E11D2E]/30'
            : 'border-[#2A2A2A] hover:border-[#E11D2E]/60'
        }`}
        aria-label={name}
        title={name}
      >
        {iconUrl && !imgError ? (
          ...
          <span className="font-mono text-[10px] font-bold text-[#A1A1AA] uppercase">
            {name.slice(0, 2)}
          </span>
        )}
      </div>
      ...
      <div
        role="tooltip"
        className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2.5 py-1 bg-[#181818] border border-[#3A3A3A] text-white text-[11px] font-mono rounded shadow-2xl whitespace-nowrap pointer-events-none transition-all duration-200 z-40 flex flex-col items-center ${
          isHovered
      ...
      >
        ...
        <span className="w-1.5 h-1.5 bg-[#181818] border-r border-b border-[#3A3A3A] transform rotate-45 -mb-1 mt-0.5" />
      </div>
    </div>
  );
}
